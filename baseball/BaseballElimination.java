...
public class BaseballElimination {
    ...
    public boolean isEliminated(String team) {
        ...
        FlowNetwork flowgraph = new FlowNetwork(2 + teamsNumber * teamsNumber + teamsNumber);
        ...
        for (i = 0; i < teamsNumber; i++) {
            ...
            for (count = i + 1; count < teamsNumber; count++) {
                ...
                if (games[i][count] != 0) {
                    flowgraph.addEdge(
                            new FlowEdge(0, 1 + i * teamsNumber + count, games[i][count]));
                    flowgraph.addEdge(new FlowEdge(1 + i * teamsNumber + count, 1 + square + i,
                                                   Double.POSITIVE_INFINITY));
                    flowgraph.addEdge(new FlowEdge(1 + i * teamsNumber + count, 1 + square + count,
                                                   Double.POSITIVE_INFINITY));
                }
            }
        }
        ...
        for (i = 0; i < teamsNumber; i++) {
            if (i == id) continue;
            if ((total - wins[i]) < 0) flowgraph.addEdge(
                    new FlowEdge(1 + square + teamsNumber, 1 + square + i, wins[i] - total));
            else flowgraph.addEdge(
                    new FlowEdge(1 + square + i, 1 + square + teamsNumber, total - wins[i]));
        }
