...
public class KdTree {
    ...
    private Node2d nearest(Node2d current, Point2D p, Node2d nearest) {
        if (current == null || nearest.point.distanceTo(p) < current.point.distanceTo(p)) {
            return nearest;
        }
        ...
        if (left.point.distanceTo(p) < nearest.point.distanceTo(p)) {
            nearest = left;
        }

        Node2d right = nearest(current.right, p, nearest);
        if (right.point.distanceTo(p) < nearest.point.distanceTo(p)) {
            nearest = right;
        }
